...
test("Test LangChain V2 tracer", async () => {
    ...
    await tracer.handleChatModelStart({ name: "testChatModel" }, [[new HumanChatMessage("I'm a human.")]], chatRunId);
    await tracer.handleLLMEnd({ generations: [[]] }, chatRunId);
    await tracer.handleToolEnd("output", toolRunId);
    const llmRunId2 = uuid.v4();
    await tracer.handleLLMStart({ name: "test2" }, ["test"], llmRunId2, chainRunId);
