...
test("Test nested runs", async () => {
    ...
    await tracer.handleToolStart({ name: "test_tool" }, "test", toolRunId, chainRunId);
    await tracer.handleLLMStart({ name: "test_llm_child_run" }, ["test"], llmRunId, toolRunId);
    await tracer.handleLLMEnd({ generations: [[]] }, llmRunId);
    await tracer.handleToolEnd("output", toolRunId);
    const llmRunId2 = uuid.v4();
    await tracer.handleLLMStart({ name: "test_llm2" }, ["test"], llmRunId2, chainRunId);
