...
async function getResponse(request, config) {
  ...
  const fetch = async (...args) => {
    const { default: doFetch, Response } = await import("node-fetch");
    return { doFetch, Response };
  };

  function reformatRequest(req) {
    try {
      // Get body content from buffer and parse it to JSON
      const bodyContent = JSON.parse(req.body.toString());

      // Get headers from the original Request object
      const headers = {};
      for (let pair of req.headers.entries()) {
  ...
  try {
    const { doFetch, Response } = await fetch();
    const { url, options } = reformatRequest(request);
    stageOne = await doFetch(url, options);
  } catch (e) {
    console.error(e);
  ...
  if (stageOne.status >= 200 && stageOne.status !== 204) {
    if (config.responseType === "stream") {
      ...
      // GPT: This function expects stageOne.body to be a ReadableStream
